-- =============================================================================
-- Aura — Reembolsos (refunds) + bucket Storage receipts + rede de indicações
-- =============================================================================

-- -----------------------------------------------------------------------------
-- 1) Indicação em cadeia: quem indicou esta utilizadora (opcional)
-- -----------------------------------------------------------------------------
...
-- Permite ver dados básicos de quem foi indicado por mim (para lista de rede)
...
-- Colunas extra se a tabela refunds já existia só com user_id/status/receipt_path
...
-- -----------------------------------------------------------------------------
-- 3) Bucket privado receipts (caminho {user_id}/...)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 4) RPC: estatísticas da rede (indicados diretos + total na árvore)
-- -----------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.my_referral_network_stats() IS
  'Conta indicados diretos e tamanho total da rede (subárvore) da utilizadora autenticada.';

-- -----------------------------------------------------------------------------
-- 5) RPC: lista segura de indicados (para mensagens no app)
-- -----------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.list_my_referrals() IS
  'Perfis indicados diretamente pela utilizadora autenticada (dados mínimos).';
